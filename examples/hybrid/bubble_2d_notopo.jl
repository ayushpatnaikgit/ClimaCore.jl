...
(hv_center_space, hv_face_space) = hvspace_2D((-500, 500), (0, 1000), 4, 25, 6;
                                            stretch = Meshes.Uniform(), warp_fn=nothing);
...
function init_dry_rising_bubble_2d(x, z)
    ...
    r_c = 250.0
    θ_b = 300.0
    # Set thermal perturbation to zero
    θ_c = 0.0
    cp_d = C_p
    cv_d = C_v
    p_0 = MSLP
    ...
    # Set UVector ~= 0 to generate horizontal flows
    return (ρ = ρ, ρθ = ρθ, ρuₕ = ρ * Geometry.UVector(10.0))
end
