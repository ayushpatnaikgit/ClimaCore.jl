...
function rhs_invariant!(dY, Y, _, t)
  ...
  f_upwind_product3 = Operators.Upwind3rdOrderBiasedProductC2F(
    ...
    # =============
    ∂x∂ξᶠ = hv_face_space.face_local_geometry.∂x∂ξ
    ∂ξ∂xᶠ = hv_face_space.face_local_geometry.∂ξ∂x
    f_x = @. ∂x∂ξᶠ.components.data.:3 * ∂ξ∂xᶠ.components.data.:1 
    f_z = @. -1.0
    n̂ = @. Geometry.UWVector(f_x, f_z)
    n̂ = Fields.Field(n̂,hv_face_space)
    sfc_unit_normal = @. -n̂ / norm(n̂)
    sfc_unit_normal = Fields.level(sfc_unit_normal, ClimaCore.Utilities.half)
    covariant_sfn = @. Geometry.transform(Geometry.Covariant13Axis(), sfc_unit_normal)

    u_1_base = Geometry.covariant1.(Fields.level(fuₕ,ClimaCore.Utilities.half), Fields.level(Fields.local_geometry_field(hv_face_space), ClimaCore.Utilities.half))
    u¹_base = Geometry.contravariant1.(Fields.level(fuₕ,ClimaCore.Utilities.half), Fields.level(Fields.local_geometry_field(hv_face_space), ClimaCore.Utilities.half))
    ...
    g31 = gⁱʲ.components.data.:2
    term1 = Geometry.Covariant3Vector.(-1 .* covariant_sfn.components.data.:1 .* u¹_base ./ g33 ./ covariant_sfn.components.data.:2)
    #@show extrema(parent(g33))
    #@show extrema(parent(g31))
    u_3_base = Geometry.Covariant3Vector.(-1 .* g31 .* u_1_base ./ g33) #+ term1  # fw = -g^31 cuₕ/ g^33
    ...
    #bcb = @. Geometry.WVector.(-9.81 * ρ₁)
    #bct = @. Geometry.WVector.(-9.81 * ρₙ)
    vgradc2fP = Operators.GradientC2F(
     #   bottom = Operators.SetGradient(bcb),
     #   top = Operators.SetGradient(bct),
    )
