...
uₕ_local = map(_ -> Geometry.UWVector(20.0, 0.0), coords)
...
anim = Plots.@animate for u in sol.u
  cw = If2c.(u.w)
  cuw = Geometry.Covariant13Vector.(u.uₕ) .+ Geometry.Covariant13Vector.(cw)
  Plots.plot(Geometry.WVector.(cuw))
end
...
anim = Plots.@animate for u in sol.u
  cw = If2c.(u.w)
  cuw = Geometry.Covariant13Vector.(u.uₕ) .+ Geometry.Covariant13Vector.(cw)
  Plots.plot(Geometry.UVector.(cuw) .- Geometry.UVector.(20.0))
end
